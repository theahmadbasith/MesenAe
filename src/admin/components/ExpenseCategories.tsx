...
import { useDbQuery, dbInsert, dbUpdate, dbDelete, type ExpenseCategory } from '@/hooks/db-hooks';
import { Tag, Plus, Edit2, Trash2, Loader2, ArrowLeft, GripVertical, Check, Palette } from 'lucide-react';
...
interface SortableExpenseCategoryRowProps {
  c: ExpenseCategory;
...
function SortableExpenseCategoryRow({ c, hasEditAccess, onEdit, onDelete }: SortableExpenseCategoryRowProps) {
      ...
      {hasEditAccess && (
        <div className="flex items-center gap-1 shrink-0">
          ...
          <Button 
            ...
            onClick={() => onDelete(c.id!.toString())}
          >
...
export default function ExpenseCategories({ open, onOpenChange }: ExpenseCategoriesProps) {
  const dbCategories = useDbQuery<ExpenseCategory>('expenseCategories') || [];
  const [categories, setCategories] = useState<ExpenseCategory[]>([]);
  const [view, setView] = useState<'list' | 'add' | 'edit'>('list');

  // Filter out soft-deleted items
  const activeDbCategories = React.useMemo(() => {
    return dbCategories;
  }, [dbCategories]);

  useEffect(() => {
    if (activeDbCategories.length > 0) {
      const sorted = [...activeDbCategories].sort((a, b) => {
    ...
    if (over && active.id !== over.id) {
      ...
      try {
        const promises = newOrder.map((item, idx) => {
          if (item.order !== idx) {
            return dbUpdate('expenseCategories', item.id!, { order: idx });
          }
          return Promise.resolve();
        });
        await Promise.all(promises);
        toast.success('Urutan kategori pengeluaran berhasil diperbarui');
      } catch (err) {
        toast.error('Gagal memperbarui urutan');
        setCategories(activeDbCategories); // Revert
      }
    }
  };
  ...
  const hasEditAccess = isAdmin || canEdit('expenses');
  ...
  const handleDelete = (id: string) => {
    if (!hasEditAccess) {
      toast.error('Akses ditolak. Anda tidak memiliki izin untuk mengelola kategori pengeluaran.');
      return;
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      await dbDelete('expenseCategories', deleteConfirmId);
  ...
  const saveCat = async (e: React.FormEvent) => {
    ...
    try {
      if (editCategory) {
        await dbUpdate('expenseCategories', editCategory.id!, { name: name.trim(), icon: catIcon, color: catColor });
      } else {
        await dbInsert('expenseCategories', { 
          name: name.trim(), 
          icon: catIcon, 
          color: catColor, 
          isDefault: 0,
      ...
      setView('list');
      toast.success('Kategori pengeluaran berhasil disimpan');
    } catch (error: any) {
          ...
          <div className="px-6 py-5 border-b border-border/50 bg-muted/10 shrink-0 flex items-center justify-between">
            <div>
              <DialogTitle className="text-base font-bold flex items-center gap-2 text-foreground leading-none">
                ...
                {view === 'list' ? 'Kelola Kategori Pengeluaran' : view === 'add' ? 'Tambah Kategori Pengeluaran' : 'Edit Kategori Pengeluaran'}
              </DialogTitle>
              <p className="text-[11px] text-muted-foreground mt-1.5 leading-none">
                {view === 'list' ? (
                  ...
                  'Ubah rincian kategori pengeluaran.'
                )}
              </p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 custom-scrollbar min-h-0">
            {!hasEditAccess && (
              <div className="flex items-center gap-2.5 px-4 py-3 mb-4 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/30 text-xs text-amber-800 dark:text-amber-300 select-none">
                ...
                <span><strong>Mode Lihat Saja:</strong> Anda tidak memiliki izin untuk mengelola kategori pengeluaran.</span>
              </div>
            )}
            ...
            {view === 'list' ? (
              ...
              !categories?.length ? (
                <div className="flex flex-col items-center py-16 text-center text-muted-foreground gap-3">
                  ...
                  <p className="text-xs text-muted-foreground max-w-sm">Belum ada kategori pengeluaran. Silahkan tambahkan kategori baru.</p>
                  {hasEditAccess && (
              ...
              <form onSubmit={saveCat} className="grid grid-cols-1 md:grid-cols-2 gap-6 py-2">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1"><Palette className="w-3.5 h-3.5" /> Pilih Warna Preset</Label>
                    <div className="flex flex-wrap gap-2.5 p-3.5 border border-border/50 bg-muted/20 rounded-xl">
                      {PRESET_COLORS.map(color => (
                        <button 
                          ...
                          className={cn('w-9 h-9 rounded-full border-2 transition-all flex items-center justify-center hover:scale-105 active:scale-95 shadow-sm',
                            catColor.toLowerCase() === color.toLowerCase() ? 'border-foreground scale-110 shadow-md ring-2 ring-primary/20' : 'border-transparent'
                          )}>
                          {catColor.toLowerCase() === color.toLowerCase() && <Check className="w-4 h-4 text-white drop-shadow-md" />}
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            <div className="flex flex-col items-center text-center space-y-4 pt-2">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground leading-normal">
                  Apakah Anda yakin ingin menghapus kategori pengeluaran ini secara permanen? Tindakan ini tidak dapat dibatalkan.
                </p>
              </div>
            </div>
